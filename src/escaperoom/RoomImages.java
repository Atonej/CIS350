...
public class RoomImages {
	...
	public String setwalkImage(int walk) {
		...
		for (int i = 0; i < forward.size() - 1; i++) {
			...
			if (pass) {
				if (walk < 10) {
					pic = new File(
							"src/00" + Integer.toString(walk) + ".jpg");
				} else if (walk < 100) {
					pic = new File(
							"src/0" + Integer.toString(walk) + ".jpg");
				} else if (walk < 200) {
					pic = new File(
							"src/" + Integer.toString(walk) + ".jpg");
				}
		...
		try { // use an url for the undo button
			this.walk = new ImageIcon(ImageIO.read(pic));
		} catch (MalformedURLException mue) { // error of form to frame
			mue.printStackTrace();
			this.walk = null;
			str = null;

		} catch (IOException ioe) { // can't print
			this.walk = null;
			str = null;
	...
	public ImageIcon darkRoom(int num) {
		...
		try { // use an url for the undo button
			walk = new ImageIcon(ImageIO.read(dark[num]));
		} catch (MalformedURLException mue) { // error of form to frame
			mue.printStackTrace();
		} catch (IOException ioe) { // can't print
			ioe.printStackTrace();
	...
	public ImageIcon getGame1() {
		...
		try { // use an url for the undo button
			game3 = new ImageIcon(ImageIO.read(pic));
		} catch (MalformedURLException mue) { // error of form to frame
			mue.printStackTrace();
		} catch (IOException ioe) { // can't print
			ioe.printStackTrace();
	...
	public ImageIcon getGame2() {
		...
		try { // use an url for the undo button
			game2 = new ImageIcon(ImageIO.read(pic));
		} catch (MalformedURLException mue) { // error of form to frame
			mue.printStackTrace();
		} catch (IOException ioe) { // can't print
			ioe.printStackTrace();
	...
	public ImageIcon getGame3() {
		...
		try { // use an url for the undo button
			game1 = new ImageIcon(ImageIO.read(pic));
		} catch (MalformedURLException mue) { // error of form to frame
			mue.printStackTrace();
		} catch (IOException ioe) { // can't print
			ioe.printStackTrace();
	...
	public ImageIcon getGame4() {
		...
		try { // use an url for the undo button
			game4 = new ImageIcon(ImageIO.read(pic));
		} catch (MalformedURLException mue) { // error of form to frame
			mue.printStackTrace();
		} catch (IOException ioe) { // can't print
			ioe.printStackTrace();
